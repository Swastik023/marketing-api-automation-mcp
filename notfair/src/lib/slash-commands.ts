 ...
 *
 * Every command here is handled CLIENT-SIDE by the chat composer.
 * Neither `claude -p` nor `codex exec` interprets slash commands in the
 * prompt, so the catalog lists only commands NotFair itself executes.
 */
