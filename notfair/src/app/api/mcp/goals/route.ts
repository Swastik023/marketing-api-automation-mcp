...
/**
 * MCP server endpoint for goal agents (the notfair-goals server).
 *
 * Transport: streamable-http with single-shot JSON responses (no SSE; the
 * goal tools are short-lived and don't stream). Clients accept
 * either; plain JSON keeps the code small.
 *
 * Auth: shared secret in ~/.notfair/mcp-server-secret. Loopback-only +
 * single-user CLI, so a shared bearer is sufficient. Agents pick up the
 * secret written at agent-provision time.
 */
...
export async function GET(): Promise<Response> {
  ...
  return NextResponse.json({
    ...
    note: "POST JSON-RPC with Bearer auth. See ~/.notfair/mcp-server-secret.",
  });
}
