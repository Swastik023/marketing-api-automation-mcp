...
export async function GET(
  ...
  const cursor = offsetParam ? Number(offsetParam) : 0;
  const validOffset = Number.isFinite(cursor) && cursor >= 0 ? cursor : 0;

  const { events, cursor: newCursor } = readTranscriptTail(
    project.slug,
    resolved.agent_id,
    threadId,
    validOffset,
