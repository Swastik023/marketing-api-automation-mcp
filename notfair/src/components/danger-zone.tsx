...
export function DangerZone({ projectSlug, projectName }: Props) {
  ...
  function onConfirm() {
    startTransition(async () => {
      ...
      const d = r.data;
      const partial = d.agentsFailed.length;
      if (partial > 0) {
        const detail = d.agentsFailed[0]?.error ?? "";
        toast.warning(
          `Deleted, but ${partial} cleanup step${partial === 1 ? "" : "s"} failed${detail ? `: ${detail}` : "."}`,
        );
      } else {
        toast.success(`Deleted ${projectName}. ${d.agents.length} goals removed.`);
      }
      setOpen(false);
      router.push("/");
      router.refresh();
        ...
        <div className="flex items-start gap-4 p-[18px]">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[12.5px] leading-snug text-[hsl(var(--notfair-ink-4))]">
              Removes every goal, its agent, and every thread of
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete the items below from NotFair&apos;s
