...
const COMPONENTS: Components = {
  ...
  th: ({ className, ...props }) => (
    <th
      className={cn("bg-[hsl(var(--notfair-surface-2))] px-2 py-1.5 text-left font-medium first:rounded-l-md last:rounded-r-md", className)}
      {...props}
    />
  ),
  td: ({ className, ...props }) => (
  ...
  pre: ({ className, ...props }) => (
    <pre
      className={cn(
        "mb-3 max-w-full overflow-x-auto rounded-md bg-muted/60 p-3 last:mb-0",
        className,
      )}
      {...props}
    />
