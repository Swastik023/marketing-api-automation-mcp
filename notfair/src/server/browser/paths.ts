 ...
 *
 * Layout under ~/.notfair/ (override via NOTFAIR_DATA_DIR):
 *
 ...
 * 19000-19099 range. We intentionally avoid the 18800-18899 range other tools use
...
/** Resolve the NotFair data dir (matches db.ts / agent-meta.ts convention). */
export function notfairDataDir(): string {
  return process.env.NOTFAIR_DATA_DIR ?? join(homedir(), ".notfair");
}

/** True when slug matches the lowercase-alphanum + hyphen format the rest of NotFair uses. */
 ...
 * NotFair process can reattach to a still-running Chrome without
