...
- **Codex** — installs the repository directly as `notfair@swastik-agnihotri`. `.codex-plugin/plugin.json` loads the universal MCP configuration and a portable `skills/` wrapper index that forwards to the same host-agnostic skill sources; no separate copied adapter is needed.
